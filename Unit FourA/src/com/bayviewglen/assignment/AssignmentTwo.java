				...
				while (!roundOver){
					...
					*/
					...
					while (!guessOver){
						...
						int guessCounter = 7; // to count number of guesses
						for (int i = 0; i < guessTotal; i++){ // to limit number of guesses at 7
							...
							System.out.println();
							System.out.println(playerTwo + ", you have " + guessCounter + " guesses to use, would you like to (1) solve or (2) guess a character: ");
								...
								while (!validGuessedCharacter){
									...
									for (int e = 0; e < guessedCharacter.length() && validGuessedCharacter; e++){
										if(guessedCharacter.length() != 1 && VALID_CHARACTERS.indexOf(guessedCharacter.charAt(e)) == -1){ 
											validGuessedCharacter = false; // why does an alphanumeric word not count here?
